...
\title{SpeakPrep ��� Document 5: Full Development Playbook}
...
\hypertarget{speakprep-document-5-full-development-playbook}{%
\section{SpeakPrep --- Document 5: Full Development
Playbook}\label{speakprep-document-5-full-development-playbook}}

\hypertarget{exact-prompts-git-strategy-cicd-mockups-agent-assignments}{%
\subsubsection{Exact Prompts · Git Strategy · CI/CD · Mockups · Agent
Assignments}\label{exact-prompts-git-strategy-cicd-mockups-agent-assignments}}
...
\begin{quote}
This document tells you exactly what to do, in what order, with what
commands and prompts, before and during every phase of building
SpeakPrep. Read Part 1 fully before touching anything. Every other part
is a reference --- open it when you reach that phase.
\end{quote}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{part-0-everything-to-set-up-before-writing-code}{%
\subsection{PART 0: EVERYTHING TO SET UP BEFORE WRITING
CODE}\label{part-0-everything-to-set-up-before-writing-code}}

Do this in one sitting. It takes 3--4 hours. Do it once, correctly.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{step-0.1-accounts-30-min}{%
\subsubsection{Step 0.1 --- Accounts (30
min)}\label{step-0.1-accounts-30-min}}

Create these accounts if you don't have them. Save every API key in a
password manager (Bitwarden, 1Password) immediately.
...
After creating each account, immediately grab the API key and store it.
You'll need them all in Step 0.4.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{step-0.2-local-dev-tools-30-min}{%
\subsubsection{Step 0.2 --- Local Dev Tools (30
min)}\label{step-0.2-local-dev-tools-30-min}}

Install everything before writing a single line of code. Missing tools
mid-build breaks flow.
...
\hypertarget{step-0.3-repository-setup-45-min}{%
\subsubsection{Step 0.3 --- Repository Setup (45
min)}\label{step-0.3-repository-setup-45-min}}

This is the most important setup step. Every decision here affects every
day of development.
...
\textbf{Create \passthrough{\lstinline!.gitignore!} --- critical:}
...
\textbf{Create \passthrough{\lstinline!CONTEXT.md!} --- your agent
shared memory:}
...
\textbf{Copy your docs into the repo:}
...
\textbf{First commit --- the foundation:}
...
\hypertarget{step-0.4-environment-variables-15-min}{%
\subsubsection{Step 0.4 --- Environment Variables (15
min)}\label{step-0.4-environment-variables-15-min}}
...
\hypertarget{step-0.5-pre-commit-hooks-15-min}{%
\subsubsection{Step 0.5 --- Pre-commit Hooks (15
min)}\label{step-0.5-pre-commit-hooks-15-min}}

These run automatically before every commit. They catch secrets, bad
formatting, and type errors before they hit GitHub.
...
\hypertarget{step-0.6-github-actions-cicd-45-min}{%
\subsubsection{Step 0.6 --- GitHub Actions CI/CD (45
min)}\label{step-0.6-github-actions-cicd-45-min}}

Set this up before writing any application code. Having CI from day one
means every commit is validated.

\begin{lstlisting}[language=bash]
mkdir -p .github/workflows
\end{lstlisting}

\textbf{Create \passthrough{\lstinline!.github/workflows/ci.yml!}:}
...
\textbf{Create \passthrough{\lstinline!.github/workflows/deploy.yml!}:}
...
\hypertarget{step-0.7-branch-strategy-setup-5-min}{%
\subsubsection{Step 0.7 --- Branch Strategy Setup (5
min)}\label{step-0.7-branch-strategy-setup-5-min}}
...
\hypertarget{step-0.8-uiux-mockup-timing-decision}{%
\subsubsection{Step 0.8 --- UI/UX Mockup Timing
Decision}\label{step-0.8-uiux-mockup-timing-decision}}

\textbf{Do mockups BEFORE building the frontend (Phase 5), not at the
start.}
...
\textbf{What to do NOW instead of full mockups:}
...
That's enough to guide the API design. The detailed Figma mockups come
in Phase 5.

\textbf{When you reach Phase 5, do this user flow research:} 1. Ask 5
friends to describe their dream interview practice tool (10-min call
each) 2. Record what they say about existing tools (what's annoying,
what's missing) 3. Build one Figma flow for the Voice Session page ---
that's the core experience 4. Test it with 2 users before writing
frontend code

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{step-0.9-final-pre-code-checklist}{%
\subsubsection{Step 0.9 --- Final Pre-Code
Checklist}\label{step-0.9-final-pre-code-checklist}}
...
\hypertarget{part-1-git-discipline-granular-tracking}{%
\subsection{PART 1: GIT DISCIPLINE --- GRANULAR
TRACKING}\label{part-1-git-discipline-granular-tracking}}

This is how you track every single change so you can revert anything.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{the-golden-rule-commit-every-logical-unit}{%
\subsubsection{The Golden Rule: Commit Every Logical
Unit}\label{the-golden-rule-commit-every-logical-unit}}

A ``logical unit'' is the smallest change that makes sense on its own.
Examples:
...
\hypertarget{how-to-stage-changes-granularly}{%
\subsubsection{How to Stage Changes
Granularly}\label{how-to-stage-changes-granularly}}
...
\passthrough{\lstinline!git add -p!} shows you each change and asks:
\passthrough{\lstinline!Stage this hunk [y,n,q,a,d,/,s,e,?]?!} -
\passthrough{\lstinline!y!} --- yes, stage this change -
\passthrough{\lstinline!n!} --- no, skip it -
\passthrough{\lstinline!s!} --- split into smaller chunks -
\passthrough{\lstinline!e!} --- manually edit what to stage

\textbf{Example of granular commit workflow:}
...
\hypertarget{commit-message-format-strict}{%
\subsubsection{Commit Message Format
(Strict)}\label{commit-message-format-strict}}
...
\hypertarget{how-to-revert-every-scenario}{%
\subsubsection{How to Revert --- Every
Scenario}\label{how-to-revert-every-scenario}}
...
\hypertarget{branch-workflow-per-feature}{%
\subsubsection{Branch Workflow Per
Feature}\label{branch-workflow-per-feature}}
...
\hypertarget{tagging-milestones}{%
\subsubsection{Tagging Milestones}\label{tagging-milestones}}
...
\hypertarget{part-2-agent-assignments-exact-prompts}{%
\subsection{PART 2: AGENT ASSIGNMENTS + EXACT
PROMPTS}\label{part-2-agent-assignments-exact-prompts}}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{the-agent-handoff-protocol-do-this-every-time}{%
\subsubsection{The Agent Handoff Protocol (Do This Every
Time)}\label{the-agent-handoff-protocol-do-this-every-time}}
...
\hypertarget{cursor-ide-daily-driver}{%
\subsubsection{Cursor IDE --- Daily
Driver}\label{cursor-ide-daily-driver}}

\textbf{When to use Cursor:} - Editing specific files - Adding one
function to an existing file - Debugging a specific error - Quick
refactors within a file - Understanding code you're reading

\textbf{Cursor Opening Prompt (use at start of every session):}
...
\textbf{Cursor Prompt for Adding a Function:}
...
\textbf{Cursor Prompt for Fixing a Bug:}
...
\textbf{Cursor Prompt for Understanding Code:}
...
\hypertarget{claude-code-architect-new-modules}{%
\subsubsection{Claude Code --- Architect + New
Modules}\label{claude-code-architect-new-modules}}

\textbf{When to use Claude Code:} - Building a new module from scratch
(e.g., the full VAD recorder, the scoring service) - Cross-file
refactoring (renaming something used everywhere) - Debugging complex
async issues - Writing a full test suite for a service - ``Build this
whole thing'' tasks

\textbf{Claude Code Opening Prompt:}
...
\textbf{Claude Code Prompt for a New Service:}
...
\textbf{Claude Code Prompt for Cross-File Refactor:}
...
\hypertarget{codex-cli-quick-scripts-and-utilities}{%
\subsubsection{Codex CLI --- Quick Scripts and
Utilities}\label{codex-cli-quick-scripts-and-utilities}}

\textbf{When to use Codex:} - One-off data scripts - Database seed
scripts - Quick bash utilities - Generating boilerplate (Dockerfile,
config files) - Converting data formats

\textbf{Codex Prompt Template:}
...
\hypertarget{antigravity-browser-research-agent}{%
\subsubsection{Antigravity --- Browser + Research
Agent}\label{antigravity-browser-research-agent}}

\textbf{When to use Antigravity:} - Testing the web UI during
development - Researching competitor features - Checking API
documentation - UI/UX validation (does the flow make sense?) - Web
scraping for question bank content

\textbf{Antigravity Prompt for UI Testing:}
...
\textbf{Antigravity Prompt for Research:}
...
\hypertarget{part-3-phase-by-phase-agent-prompts}{%
\subsection{PART 3: PHASE-BY-PHASE AGENT
PROMPTS}\label{part-3-phase-by-phase-agent-prompts}}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\hypertarget{phase-0-environment-foundations-week-13}{%
\subsubsection{PHASE 0: Environment + Foundations (Week
1--3)}\label{phase-0-environment-foundations-week-13}}

\textbf{Git branch:}
\passthrough{\lstinline!feature/phase0-foundations!}

\hypertarget{task-0.1-install-python-dependencies}{%
\paragraph{Task 0.1: Install Python
Dependencies}\label{task-0.1-install-python-dependencies}}

\textbf{Agent: Cursor or Claude Code}
...
\hypertarget{task-0.2-first-asyncio-exercise}{%
\paragraph{Task 0.2: First asyncio
Exercise}\label{task-0.2-first-asyncio-exercise}}
...
\hypertarget{task-0.3-first-websocket-exercise}{%
\paragraph{Task 0.3: First WebSocket
Exercise}\label{task-0.3-first-websocket-exercise}}

\textbf{Agent: Claude Code}
...
\hypertarget{task-0.4-audio-byte-level-exercise}{%
\paragraph{Task 0.4: Audio Byte-Level
Exercise}\label{task-0.4-audio-byte-level-exercise}}
...
\textbf{Phase 0 completion commit:}
...
\textbf{Update CONTEXT.md before Phase 1:}
...
\hypertarget{phase-1-local-voice-pipeline-week-46}{%
\subsubsection{PHASE 1: Local Voice Pipeline (Week
4--6)}\label{phase-1-local-voice-pipeline-week-46}}

\textbf{Git branch:}
\passthrough{\lstinline!feature/phase1-local-pipeline!}

\hypertarget{task-1.1-vad-recorder}{%
\paragraph{Task 1.1: VAD Recorder}\label{task-1.1-vad-recorder}}
...
\hypertarget{task-1.2-faster-whisper-asr-service}{%
\paragraph{Task 1.2: Faster-Whisper ASR
Service}\label{task-1.2-faster-whisper-asr-service}}
...
\hypertarget{task-1.3-groq-llm-service}{%
\paragraph{Task 1.3: Groq LLM Service}\label{task-1.3-groq-llm-service}}
...
\hypertarget{task-1.4-local-pipeline-integration-toy}{%
\paragraph{Task 1.4: Local Pipeline Integration
(Toy)}\label{task-1.4-local-pipeline-integration-toy}}
...
\hypertarget{phase-2-real-time-streaming-week-79}{%
\subsubsection{PHASE 2: Real-Time Streaming (Week
7--9)}\label{phase-2-real-time-streaming-week-79}}

\textbf{Git branch:}
\passthrough{\lstinline!feature/phase2-websocket-streaming!}

\hypertarget{task-2.1-websocket-voice-handler}{%
\paragraph{Task 2.1: WebSocket Voice
Handler}\label{task-2.1-websocket-voice-handler}}
...
\hypertarget{task-2.2-deepgram-streaming-asr}{%
\paragraph{Task 2.2: Deepgram Streaming
ASR}\label{task-2.2-deepgram-streaming-asr}}
...
\hypertarget{task-2.3-kokoro-tts-service}{%
\paragraph{Task 2.3: Kokoro TTS
Service}\label{task-2.3-kokoro-tts-service}}
...
\hypertarget{task-2.4-simple-browser-client}{%
\paragraph{Task 2.4: Simple Browser
Client}\label{task-2.4-simple-browser-client}}

\textbf{Agent: Antigravity or manual}
...
\hypertarget{phase-3-interview-intelligence-week-1012}{%
\subsubsection{PHASE 3: Interview Intelligence (Week
10--12)}\label{phase-3-interview-intelligence-week-1012}}

\textbf{Git branch:}
\passthrough{\lstinline!feature/phase3-interview-intelligence!}

\hypertarget{task-3.1-database-setup}{%
\paragraph{Task 3.1: Database Setup}\label{task-3.1-database-setup}}
...
\hypertarget{task-3.2-resume-parser}{%
\paragraph{Task 3.2: Resume Parser}\label{task-3.2-resume-parser}}
...
\hypertarget{task-3.3-question-bank-elo-selector}{%
\paragraph{Task 3.3: Question Bank + ELO
Selector}\label{task-3.3-question-bank-elo-selector}}
...
\hypertarget{task-3.4-interview-ai-system-prompts}{%
\paragraph{Task 3.4: Interview AI System
Prompts}\label{task-3.4-interview-ai-system-prompts}}
...
\hypertarget{task-3.5-scoring-service}{%
\paragraph{Task 3.5: Scoring Service}\label{task-3.5-scoring-service}}
...
\hypertarget{phase-4-deploy-to-production-week-1314}{%
\subsubsection{PHASE 4: Deploy to Production (Week
13--14)}\label{phase-4-deploy-to-production-week-1314}}

\textbf{Git branch:} \passthrough{\lstinline!feature/phase4-deployment!}

\hypertarget{task-4.1-docker-compose-production-config}{%
\paragraph{Task 4.1: Docker Compose Production
Config}\label{task-4.1-docker-compose-production-config}}
...
\hypertarget{task-4.2-oracle-arm-provisioning}{%
\paragraph{Task 4.2: Oracle ARM
Provisioning}\label{task-4.2-oracle-arm-provisioning}}

\textbf{Agent: Yourself (manual --- no agent)}
...
\hypertarget{phase-5-ui-mockups-frontend-week-1520}{%
\subsubsection{PHASE 5: UI Mockups + Frontend (Week
15--20)}\label{phase-5-ui-mockups-frontend-week-1520}}

\textbf{Before writing any React code --- do mockups first.}

\hypertarget{task-5.0-figma-mockups-1-week-before-phase-5-code}{%
\paragraph{Task 5.0: Figma Mockups (1 week before Phase 5
code)}\label{task-5.0-figma-mockups-1-week-before-phase-5-code}}

\textbf{Agent: Yourself + Antigravity for research}

\textbf{Antigravity research prompt:}
...
\textbf{After research, build Figma mockups for:} 1. \textbf{Voice
Session screen} (most important): microphone button, live transcript
pane, AI response text, latency indicator, turn counter, timer, filler
word counter 2. \textbf{Post-Session Report} screen: overall score ring,
5 dimension bars, per-question expandable cards 3. \textbf{Progress
Dashboard}: line chart (score trend), topic coverage grid, streak
counter

Only mock these 3 screens. Everything else (landing, auth, setup) can be
wire-framed quickly or built directly in code.

\hypertarget{task-5.1-react-frontend}{%
\paragraph{Task 5.1: React Frontend}\label{task-5.1-react-frontend}}

\textbf{Agent: Claude Code (with Cursor for components)}
...
\hypertarget{phase-6-hardening-launch-week-2124}{%
\subsubsection{PHASE 6: Hardening + Launch (Week
21--24)}\label{phase-6-hardening-launch-week-2124}}

\textbf{Agent: Claude Code for security and testing, Codex for scripts}

\textbf{Claude Code security prompt:}
...
\textbf{Codex load test script prompt:}
...
\textbf{Launch PR checklist commit:}
...
\hypertarget{part-4-daily-routine}{%
\subsection{PART 4: DAILY ROUTINE}\label{part-4-daily-routine}}

Every working day, 5 minutes at the start, 5 minutes at the end.

\hypertarget{morning-5-min}{%
\subsubsection{Morning (5 min)}\label{morning-5-min}}
...
\hypertarget{during-every-3060-min}{%
\subsubsection{During (every 30--60 min)}\label{during-every-3060-min}}
...
\hypertarget{end-of-day-5-min}{%
\subsubsection{End of Day (5 min)}\label{end-of-day-5-min}}
...
\hypertarget{weekly-30-min-every-friday}{%
\subsubsection{Weekly (30 min, every
Friday)}\label{weekly-30-min-every-friday}}
...
\emph{End of Document 5 --- Full Development Playbook}
